...
Met onze Shopify plugin is het mogelijk verschillende soorten MyParcel zendingen aanmaken en gebruik maken van meerdere vervoerders:
...
- Digitale Postzegel
- Ongefrankeerd label
- DHL For Your (eventueel met DHL vandaag) voor Nederland en Belgie
...
Voor het in gebruik nemen van de MyParcel app, moet er eerst een API key worden ingevuld. De API key is gratis te verkrijgen via <a href="https://backoffice.myparcel.nl/registration" target="_blank"> Shopinstellingen > API instellingen in de MyParcel Backoffice</a>.
Na het invullen van de API key wordt de MyParcel app gekoppeld aan jouw Shopify shop.
...
- Een andere optie is bij het printen / downloaden van een label de status op 'fulfilled' wordt gezet.
...
::: note
Let op! Bij de keuze voor DHL For You (Today) en DHL Vandaag moet er ook een afsluittijd worden meegegeven. Een label moet worden aangemaakt, het voor de afsluittijd aangemaakt is dan sturen we het mee.
:::
...
Het is mogelijk om vanuit de shopify bestellingen overzicht naar de checkout pagina te gaan van een specifieke bestelling en hier alsnog een afhaallocatie te selecten. Ga naar Shopify Bestellingen overzicht > Selecteer de bestelling waar geen afhaalpunt geselecteerd is. Rechtsboven onder Meer acties > Bekijk de bestellingstatuspagina. Vervolgens is het mogelijk om een afhaalpunt te selecteren (zie afbeelding hieronder).

<MPImg src="/documentation/shopify/nl/shopify-bestelling-status-pagina.jpg" alt="Shopify selecteer afhaalpunt" />

### Retourlabel mailen

Heeft jouw klant een bestelling geplaatst en wil hij/zij deze retour sturen? Verstuur eenvoudig een mail vanuit Shopify met het retourlabel. Optie 1 vanuit de Shopify bestellingenoverzicht open je de order. Onder meer het Meer acties dropdownmenu vind je de optie 'Retourlabel versturen' om een mail naar jouw klant te sturen. Optie 2 is vanuit het MyParcel bestellingenoverzicht selecteer je een specifieke order en bovenaan de pagina vervolgens op de knop 'Verstuur retourlabel' klikken.
