...
See [the terminology section](./readme.md#terminology) for a list of terms used throughout the PDK and this documentation.
...
Frontend actions are actions that are used in the frontend of your platform. These actions are used to communicate with the PDK backend and are used to render and update the frontend.

**Backend actions**

Backend actions are actions that are used in the backend of your platform. These actions are used to communicate with the MyParcel API and are needed for all functionality.

**Shared actions**

Shared actions are actions that can be used in both the frontend and backend of your platform.
...
See [the full list of actions] for more information.

[the full list of actions]: https://github.com/myparcelnl/pdk/blob/main/config/actions.php
...
::: tip
📚 For more, check out our actual [WooCommerce plugin](https://github.com/myparcelnl/woocommerce/tree/alpha), which is built with the PDK.
:::
...
Repeat this for all required classes and any other classes you want to implement yourself. Keep in mind you can only extend interfaces, not classes.
...
This allows for easy access to the plugin name, title, version, path and URL. You can use this file along with the config file to instantiate the PDK.
...
Create all required classes and register them in your PDK [configuration]. You can now use the PDK in your own code. For more information, see the [PDK frontend documentation].

[template configuration]: https://github.com/myparcelnl/pdk/blob/main/config/pdk-template.php
[PDK frontend documentation]: ../20.frontend/20.frontend.md
