...
Currently, our system supports only PostNL labels in ZPL format. Additionally, it's important to note that you can request just one label at a time.
Labels are available for NL and EU countries. If you request a rest of world label, the system will return a label in PDF format.
Within the Netherlands, our system supports both normal packages and mailbox packages.
